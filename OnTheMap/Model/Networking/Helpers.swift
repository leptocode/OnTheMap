...
class Helpers {
    ...
    class func taskForGETRequest<ResponseType: Decodable>(url: URL, apiType: String, responseType: ResponseType.Type, completion: @escaping (ResponseType?, Error?) -> Void) {
        ...
        if apiType == "Udacity" {
            ...
            request.addValue(APIHeaders.ApplicationId, forHTTPHeaderField: "X-Parse-Application-Id")
            request.addValue(APIHeaders.APIKey, forHTTPHeaderField: "X-Parse-REST-API-Key")
        }
